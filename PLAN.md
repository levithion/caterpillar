...
- [x] Full dataset (all 10 CSVs, schema below) generated by `scripts/generate_data.py` —
      deterministic/reproducible (verified: identical `md5sum` across reruns), additive
      only (existing columns/files untouched, backend `load_all()` still loads clean)
- [ ] Coaching, ergonomics, and environment **endpoints/UI** (outcomes 6–8) — data exists,
      backend routers + frontend panels not started; see Phase 2
